...
%     if A is a matrix.
% 
% imageH - the number of columns of the pixel matrix
%
% imageV - the number of rows of the pixel matrix
%
...
function [xp, tp, niter] = tveq_newton(x0, t0, A, At, b, tau, newtontol, newtonmaxiter, slqtol, slqmaxiter,imageH,imageV) 
...
Dv = spdiags([reshape([-ones(imageV-1,imageH); zeros(1,imageH)],N,1) ...
              reshape([zeros(1,imageH); ones(imageV-1,imageH)],N,1)], [0 1], N, N);
Dh = spdiags([reshape([-ones(imageV,imageH-1) zeros(imageV,1)],N,1) ...
              reshape([zeros(imageV,1) ones(imageV,imageH-1)],N,1)], [0 n], N, N);

% auxillary matrices for preconditioning
Mdv = spdiags([reshape([ones(imageV-1,imageH); zeros(1,imageH)],N,1) ...
               reshape([zeros(1,imageH); ones(imageV-1,imageH)],N,1)], [0 1], N, N);
Mdh = spdiags([reshape([ones(imageV,imageH-1) zeros(imageV,1)],N,1) ...
               reshape([zeros(imageV,1) ones(imageV,imageH-1)],N,1)], [0 n], N, N);
Mmd = reshape([ones(imageV-1,imageH-1) zeros(imageV-1,1); zeros(1,imageH)],N,1);
