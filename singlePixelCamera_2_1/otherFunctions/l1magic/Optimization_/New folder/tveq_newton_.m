...
Dv = spdiags([reshape([-ones(n,n); zeros(1,n)],N,1) ...
  reshape([zeros(1,n); ones(n,n)],N,1)], [0 1], N, N);

Dh = spdiags([reshape([-ones(n+1,n-1) zeros(n+1,1)],N,1) ...
  reshape([zeros(n+1,1) ones(n+1,n-1)],N,1)], [0 n], N, N);

% auxillary matrices for preconditioning
Mdv = spdiags([reshape([ones(n,n); zeros(1,n)],N,1) ...
  reshape([zeros(1,n); ones(n,n)],N,1)], [0 1], N, N);
Mdh = spdiags([reshape([ones(n+1,n-1) zeros(n+1,1)],N,1) ...
  reshape([zeros(n+1,1) ones(n+1,n-1)],N,1)], [0 n], N, N);
Mmd = reshape([ones(n,n-1) zeros(n,1); zeros(1,n)],N,1);
